...
int find_short_proc(struct Process processes[], int n, int curr_time) {
    int min_bur_time = 1e9; 
    int short_in = -1;

    for (int i = 0; i < n; i++) {
        if (processes[i].arr_time <= curr_time && !processes[i].iscomp) {
            if (processes[i].bur_time < min_bur_time) {
                min_bur_time = processes[i].bur_time;
                short_in = i;
            }
        }
    }
...
void calc_time(struct Process processes[], int n) {
    ...
    while (iscomp_processes < n) {
        int short_in = find_short_proc(processes, n, curr_time);

        if (short_in != -1) {
            struct Process *curr_proc = &processes[short_in];
